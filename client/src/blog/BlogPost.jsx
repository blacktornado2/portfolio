...
function TableOfContents() {
  ...
  return (
    <aside className="sticky top-20">
      <p className="font-mono text-[10px] uppercase tracking-[0.1em] text-[var(--text-3)] mb-3.5">
        On this page
      </p>
            ...
            <a
              ...
              className={`block text-[12.5px] leading-snug py-1.5 border-l-2 transition-all duration-150 ${
                ...
                active === id
                  ...
                  : "border-[var(--border)] text-[var(--text-2)] hover:text-[var(--accent)] hover:border-[var(--accent)]"
              }`}
            >
              {label}
            </a>
      ...
      <div className="flex items-center gap-2.5 mb-6">
        ...
        />
        <div>
          <div className="font-syne font-bold text-[13px] text-[var(--text-1)]">Ankit Bhardwaj</div>
          <div className="font-mono text-[11px] text-[var(--text-3)]">Full-Stack Dev</div>
        </div>
      </div>

      <p className="font-mono text-[10px] uppercase tracking-[0.1em] text-[var(--text-3)] mb-2.5">
        Share
      </p>
      <div className="flex flex-wrap gap-1.5">
        <a
          href="#"
          className="inline-flex items-center gap-1.5 font-mono text-[11px] text-[var(--text-2)] bg-[var(--surface)] border border-[var(--border)] rounded px-2.5 py-1.5 hover:border-[var(--accent)] hover:text-[var(--accent)] transition-colors duration-150"
        >
...
export default function BlogPost() {
  ...
  if (loading) {
    return (
      <main className="bg-[var(--bg)] min-h-screen text-[var(--text-1)] flex items-center justify-center">
        <div className="font-mono text-[var(--text-3)]">Loading…</div>
      </main>
    );
  }
      ...
      >
        <article>
          <div className="flex flex-wrap items-center gap-2 mb-6">
            ...
            <span className="font-mono text-[12px] text-[var(--text-2)]">{new Date(post.publishedAt).toLocaleDateString()}</span>
            <span className="text-[var(--text-3)] text-xs">·</span>
            <span className="font-mono text-[12px] text-[var(--text-2)]">{post.readTime}</span>
          </div>

          <h1 className="font-syne font-extrabold text-[clamp(26px,5vw,42px)] leading-[1.15] tracking-[-0.03em] mb-5">
            {post.title}
          </h1>
          <p className="text-lg text-[var(--text-2)] leading-[1.7] mb-9 font-light" style={{ textWrap: "pretty" }}>
            {post.summary}
          </p>
          <hr className="border-[var(--border)] mb-9" />
          ...
          <div className="prose prose-invert max-w-none mb-12">
            <ReactMarkdown
              ...
              components={{
                ...
                p: ({ node, ...props }) => (
                  <p className="text-[15px] text-[var(--text-1)]/80 leading-[1.8] mb-5" {...props} />
                ),
                code: ({ node, className, children, ...props }) => {
                  const match = /language-(\w+)/.exec(className || "");
                  if (!match) {
                    return <code className="font-mono text-[13px] bg-[var(--surface)] border border-[var(--border)] px-1.5 py-0.5 rounded text-[var(--accent)]" {...props}>{children}</code>;
                  }
                  const lang = match[1];
                ...
                pre: ({ node, ...props }) => (
                  <div className="bg-[var(--surface)] rounded-xl border border-[var(--border)] overflow-hidden !my-8 shadow-lg">
                    <div className="bg-[#161616] px-4 py-2.5 flex items-center gap-1.5 border-b border-[var(--border)]">
                      <span className="w-3 h-3 rounded-full bg-[#EF4444]" />
                      <span className="w-3 h-3 rounded-full bg-[#F59E0B]" />
                      <span className="w-3 h-3 rounded-full bg-[#22C55E]" />
                    </div>
                    <pre className="!m-0 p-5 font-mono text-[13.5px] leading-[1.7] text-[var(--text-2)] overflow-x-auto" {...props} />
                  </div>
                ),
                a: ({ node, href, ...props }) => {
          ...
          <div className="mt-16 pt-8 border-t border-[var(--border)]">
            ...
            <div>
              ...
              <div className="space-y-4">
                {comments.length === 0 ? (
                  <p className="text-[var(--text-3)] text-sm">No comments yet. Be the first!</p>
                ) : (
                  comments.map((comment) => (
                    <div key={comment.id} className="bg-[var(--surface)] border border-[var(--border)] rounded p-4">
                      ...
                      <p className="text-[15px] text-[var(--text-1)]/80 leading-relaxed">{comment.body}</p>
                    </div>
                  ))
                )}
              </div>
